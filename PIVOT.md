...
- **Progress:** RC-004 is accepted as design only, and RC-005 is complete for the monorepo/typecheck contract. The RC-002 receipt requirement remains unchanged; no task that runs or supervises real Distill can start until RC-002 is accepted, and the old prompt must not be restarted.
- **Next action:** Proceed to RC-006, “Start Elysia API in the container.” RC-007 and other tasks that execute or supervise real Distill remain gated on RC-002.
