...
- **Dependency order:** RC-008 remains incomplete. RC-010 is resequenced to start from RC-006 and the verified SQLite/named-volume substrate slice because its explicit Start condition is “Persistent API,” which this slice meets; its auth session storage will exercise that real persistent API. This change does not certify workspace/history/profile durability or accept RC-008. RC-009 remains gated by full RC-008 and unresolved RC-002. The remaining RC-008 persistence proof stays required for its own eventual acceptance.
